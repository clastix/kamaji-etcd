...
| certManager.caBundle.enabled | bool | `false` | Build etcd's trusted-ca-file as a merged bundle of CA sources via an init container in the etcd Pods, instead of relying on trust-manager. Required when certManager.issuerRef points to an Issuer backed by a CA that is itself signed by another, external CA (a chained/intermediate setup): in that case cert-manager only ever propagates a single certificate into a Certificate's secret ca.crt field, so etcd would trust either the intermediate or the root, never both. The init container reads each source Secret directly (across namespaces, via the etcd Pods' own ServiceAccount RBAC - no fixed "trust namespace" restriction to work around) and writes the concatenated PEM into an emptyDir which etcd uses as its trusted-ca-file. It only re-syncs when a Pod (re)starts, not continuously - if a source CA rotates, perform a rollout restart of the etcd StatefulSet to refresh it. Unlike a Helm hook, this is compatible with `helm install --wait`: the init container waits for cert-manager to populate the source Secrets, restarting until they are available. |
| certManager.caBundle.image | object | `{"pullPolicy":"IfNotPresent","repository":"clastix/kubectl","tag":"v1.35"}` | Container image for the ca-bundle init container. Only needs a POSIX shell and kubectl. |
