...
![Version: 0.18.0](https://img.shields.io/badge/Version-0.18.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 3.6.12](https://img.shields.io/badge/AppVersion-3.6.12-informational?style=flat-square)
...
| certManager.caBundle.additionalCASecrets | list | `[]` | Additional CA Secrets to concatenate in, alongside this chart's own CA (certManager.ca), which is always included first - e.g. the external root CA that certManager.issuerRef chains up to. Unlike a cert-manager-issued Secret, a manually provisioned one may only have `tls.crt` (no `ca.crt`) - set `key` per entry to match whatever that specific Secret actually has. |
| certManager.caBundle.enabled | bool | `false` | Build etcd's trusted-ca-file as a merged bundle of CA sources via a Helm hook Job (runs on every install/upgrade), instead of relying on trust-manager. Required when certManager.issuerRef points to an Issuer backed by a CA that is itself signed by another, external CA (a chained/intermediate setup): in that case cert-manager only ever propagates a single certificate into a Certificate's secret ca.crt field, so etcd would trust either the intermediate or the root, never both. The Job reads each source Secret directly (across namespaces, via its own RBAC - no fixed "trust namespace" restriction to work around) and writes the concatenated PEM into a plain Secret in this release's namespace, which etcd mounts like any other cert. It only re-syncs when the Job re-runs (i.e. on `helm upgrade`), not continuously - if a source CA rotates between upgrades, re-run `helm upgrade` (even a no-op one) to refresh it. This runs as a post-install,post-upgrade hook, so the etcd StatefulSet is applied first and its Pods will briefly sit in ContainerCreating until the Job creates the bundle Secret they mount - the same eventually-consistent pattern the chart already relies on while cert-manager issues its own certificates. |
| certManager.caBundle.image | object | `{"pullPolicy":"IfNotPresent","repository":"clastix/kubectl","tag":"v1.35"}` | Container image for the merge Job. Only needs a POSIX shell and kubectl. |
| certManager.caBundle.ownCAKey | string | `"tls.crt"` | Secret data key holding this chart's own CA certificate (certManager.ca). When a cert-manager-issued CA Certificate's Secret pointing to an already existing CA, the `ca.crt` key contains the root certificate, and `tls.crt` the intermediate one, so the default is pointing to the latter; override only if certManager.ca ends up backed by something that doesn't follow that convention. |
